...
module blockmem(
  ...
  memory slice_0(clk, enable, reset, write_mode == 2 && bus_valid[0] ? 4'b10 : write_mode == 1 && ind == 0 ? 4'b1 : 4'b00, en,        write_mode == 2 ? bus_in[0] : in_data, {3'b0, size[5:0]}, {page_read, raddr}, {page_write, waddr}, x_out[0], mem_en[0], clear_out[0], lw[0], mid_raddr[0], mid_waddr[0]);
  memory slice_1(clk, enable, reset, write_mode == 2 && bus_valid[1] ? 4'b10 : write_mode == 1 && ind == 1 ? 4'b1 : 4'b00, mem_en[0], write_mode == 2 ? bus_in[1] : in_data, {3'b0, size[5:0]}, mid_raddr[0],       mid_waddr[0],        x_out[1], mem_en[1], clear_out[1], lw[1], mid_raddr[1], mid_waddr[1]);
  memory slice_2(clk, enable, reset, write_mode == 2 && bus_valid[2] ? 4'b10 : write_mode == 1 && ind == 2 ? 4'b1 : 4'b00, mem_en[1], write_mode == 2 ? bus_in[2] : in_data, {3'b0, size[5:0]}, mid_raddr[1],       mid_waddr[1],        x_out[2], mem_en[2], clear_out[2], lw[2], mid_raddr[2], mid_waddr[2]);
  memory slice_3(clk, enable, reset, write_mode == 2 && bus_valid[3] ? 4'b10 : write_mode == 1 && ind == 3 ? 4'b1 : 4'b00, mem_en[2], write_mode == 2 ? bus_in[3] : in_data, {3'b0, size[5:0]}, mid_raddr[2],       mid_waddr[2],        x_out[3], mem_en[3], clear_out[3], lw[3], mid_raddr[3], mid_waddr[3]);
  memory slice_4(clk, enable, reset, write_mode == 2 && bus_valid[4] ? 4'b10 : write_mode == 1 && ind == 4 ? 4'b1 : 4'b00, mem_en[3], write_mode == 2 ? bus_in[4] : in_data, {3'b0, size[5:0]}, mid_raddr[3],       mid_waddr[3],        x_out[4], mem_en[4], clear_out[4], lw[4], mid_raddr[4], mid_waddr[4]);
  memory slice_5(clk, enable, reset, write_mode == 2 && bus_valid[5] ? 4'b10 : write_mode == 1 && ind == 5 ? 4'b1 : 4'b00, mem_en[4], write_mode == 2 ? bus_in[5] : in_data, {3'b0, size[5:0]}, mid_raddr[4],       mid_waddr[4],        x_out[5], mem_en[5], clear_out[5], lw[5], mid_raddr[5], mid_waddr[5]);
  memory slice_6(clk, enable, reset, write_mode == 2 && bus_valid[6] ? 4'b10 : write_mode == 1 && ind == 6 ? 4'b1 : 4'b00, mem_en[5], write_mode == 2 ? bus_in[6] : in_data, {3'b0, size[5:0]}, mid_raddr[5],       mid_waddr[5],        x_out[6], mem_en[6], clear_out[6], lw[6], mid_raddr[6], mid_waddr[6]);
  memory slice_7(clk, enable, reset, write_mode == 2 && bus_valid[7] ? 4'b10 : write_mode == 1 && ind == 7 ? 4'b1 : 4'b00, mem_en[6], write_mode == 2 ? bus_in[7] : in_data, {3'b0, size[5:0]}, mid_raddr[6],       mid_waddr[6],        x_out[7], mem_en[7], clear_out[7], lw[7], mid_raddr[7], mid_waddr[7]);
